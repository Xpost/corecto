...
<!-- 		  Tabla -->
		 ...
		 <hr style="margin: 7px 0px 5px;"> 
	<div class="row-fluid" id="detailHistoryDiv" style="display: none;">
<!-- 		  Tabla -->
		     		<div style="" align="center">
			    		<div style="height: 1px" ></div>
	  						 <table style="text-align: center;font-size: 16px" align="center" id="listHisIncidenciasTable">
	             			 </table>	
	             		<div id="listHisIncidenciasTableeGrid"></div>
              		</div>
   </div>
...
jQuery("#searchButton").click(function(){
	 ...
	 var tipoI = jQuery("#typeIncidenciaSearch option:selected").val();
	 if (tipoI==-1){
		 tipoI = "";
	 }
	 var estadoI = jQuery("#stateIncidenciaSearch option:selected").val(); 
	 if (estadoI==-1){
		 estadoI = "";
	 }
...
jQuery("#listIncidenciaTable").jqGrid({
	 ...
	 width : 1158,
	 height : 200,
	 colNames : ['Ver Historial','','','Tipo','','Estado','','Cliente','Fecha Inicio', 'Fecha Cierra','Descripcion','Otra info', ''],
		colModel : [ 
		        	{name: 'myac',
						 width:77, 
						 fixed:true, 
						 ...
						 resize:false, 
							...
							formatter:function(){
								...
								'<div onmouseout="jQuery(this).removeClass(\'ui-state-hover\')" onmouseover="jQuery(this).addClass(\'ui-state-hover\');"  class="ui-pg-div ui-inline-edit" style="float: left; cursor: pointer; display: block;margin-left: 4px;margin-right: 4px;" title="Modificar fila seleccionada"><i class="icon-tasks" op="edit"></i></div>'+											
								"</div>";
								}
...
function showHistory(row){
	...
	var idIncidencia = row.idincidencia;
	
    jQuery("#listHisIncidenciasTable").jqGrid('setGridParam',{url:"<c:url value="/listHisIncidencia.htm" />?cId="+idCliente+"&incId="+idIncidencia,page:1}).trigger('reloadGrid');
    jQuery("#detailHistoryDiv").show("fast");
}

jQuery("#listHisIncidenciasTable").jqGrid({
	 url: '',
	 datatype: "json", 
	 width : 1158,
	 height : 200,
	 colNames : ['','','','Fecha','','Estado','Cliente','Descripción', 'Usuario'],
		colModel : [ 
		{
			name : 'idcliente',
			index : 'idcliente',
		...
		},
		{
			...
			name : 'descripcionEstadoInc',
			index : 'descripcionEstadoInc',
			align: 'center',
			width : 75
		}, {
			name : 'nombre',
	  ...
	  multiselect: false,
	  sortable: true,
	  pager: '#listHisIncidenciasTableGrid',
	  pgbuttons: false,
	  pginput:false,
	  rownumbers : true,
	  rowList: {},
